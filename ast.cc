        ...
        if (T)
        {
            ...
            case yy::parser::token::FUNC_DEC:
                ...
                if (T->ptr[0])
                {
                    ...
                    printf("%*c无参函数\n", indent + 3, ' ');
                break;
            case yy::parser::token::PARAM_LIST:
                display(T->ptr[0], indent); //依次显示全部参数类型和名称，
                display(T->ptr[1], indent);
                break;
            case yy::parser::token::PARAM_DEC:
                printf("%*c类型：%s, 参数名：%s\n", indent, ' ', T->ptr[0]->type == yy::parser::token::INT ? "int" : "float", T->ptr[1]->type_id);
                break;
            case yy::parser::token::EXP_STMT:
                printf("%*c表达式语句：(%d)\n", indent, ' ', T->pos);
            ...
            case yy::parser::token::IF_THEN:
                ...
                break;
            case yy::parser::token::IF_THEN_ELSE:
                ...
                break;
            case yy::parser::token::DEF_LIST:
                display(T->ptr[0], indent); //显示该局部变量定义列表中的第一个
                display(T->ptr[1], indent); //显示其它局部变量定义
                break;
            case yy::parser::token::VAR_DEF:
                printf("%*c局部变量定义：(%d)\n", indent, ' ', T->pos);
                display(T->ptr[0], indent + 3); //显示变量类型
                display(T->ptr[1], indent + 3); //显示该定义的全部变量名
                break;
            case yy::parser::token::DEC_LIST:
                printf("%*c变量名：\n", indent, ' ');
                T0 = T;
                while (T0)
                {
                    if (T0->ptr[0]->kind == yy::parser::token::ID)
                        printf("%*c %s\n", indent + 6, ' ', T0->ptr[0]->type_id);
                    else if (T0->ptr[0]->kind == yy::parser::token::ASSIGNOP)
                    {
            ...
            case yy::parser::token::UMINUS:
                printf("%*c%s\n", indent, ' ', T->type_id);
                display(T->ptr[0], indent + 3);
                break;
            case yy::parser::token::FUNC_CALL:
                printf("%*c函数调用：(%d)\n", indent, ' ', T->pos);
                printf("%*c函数名：%s\n", indent + 3, ' ', T->type_id);
                display(T->ptr[0], indent + 3);
                break;
            case yy::parser::token::ARGS:
                i = 1;
                while (T)
